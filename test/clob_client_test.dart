...
class _LiveMarketRef {
  final String conditionId;
  final String tokenId;

  const _LiveMarketRef({required this.conditionId, required this.tokenId});
}

/// Fetch the top active markets by 24h volume from the Gamma API.
/// Returns market refs for markets with live CLOB token IDs.
Future<List<_LiveMarketRef>> _fetchHighVolumeMarketRefs({int limit = 5}) async {
  final uri = Uri.parse(
  ...
  final refs = <_LiveMarketRef>[];
  for (final m in markets) {
    final conditionId = m['conditionId']?.toString();
    ...
    if (conditionId != null &&
        ...
        ids.isNotEmpty) {
      refs.add(
        _LiveMarketRef(conditionId: conditionId, tokenId: ids[0].toString()),
      );
    }
  }
  return refs;
...
void main() {
  ...
  late List<_LiveMarketRef> liveMarketRefs;
  _LiveMarketRef? liveMarketRef;
  String? liveTokenId;

  setUpAll(() async {
    liveMarketRefs = await _fetchHighVolumeMarketRefs();
    liveMarketRef = liveMarketRefs.isNotEmpty ? liveMarketRefs.first : null;
    liveTokenId = liveMarketRef?.tokenId;
  });
  ...
  group('Health', () {
    ...
    test(
      'getServerTime returns a recent unix timestamp',
      () async {
  ...
  group('Markets', () {
    test(
      'getMarkets returns a page with data',
      () async {
    ...
    test(
      'getMarkets pagination cursor is returned',
      () async {
        final page = await client.getMarkets();
    ...
    test(
      'getMarket by condition ID returns valid market',
      () async {
        final page = await client.getMarkets();
  ...
  group('Orderbook', () {
    test(
      'getOrderBook returns bids and asks',
      () async {
        if (liveTokenId == null) {
    ...
    test(
      'getOrderBooks for multiple high-volume tokens returns list',
      () async {
        if (liveMarketRefs.isEmpty) {
          markTestSkipped('No active market tokens available');
          return;
        }
        final params = liveMarketRefs
            .take(3)
            .map((ref) => BookParams(tokenId: ref.tokenId))
            .toList();
  ...
  group('Pricing', () {
    test(
      'getMidpoint returns a price string',
      () async {
        if (liveTokenId == null) {
          markTestSkipped('No active market token available');
          return;
    ...
    test(
      'getPrice BUY returns a price string',
      () async {
        if (liveTokenId == null) {
          markTestSkipped('No active market token available');
          return;
    ...
    test(
      'getSpread returns a spread string',
      () async {
        if (liveTokenId == null) {
          markTestSkipped('No active market token available');
          return;
    ...
    test(
      'getLastTradePrice returns a price string',
      () async {
        if (liveTokenId == null) {
          markTestSkipped('No active market token available');
          return;
  ...
  group('Market config', () {
    test(
      'getTickSize returns a valid tick size string',
      () async {
        if (liveTokenId == null) {
          markTestSkipped('No active market token available');
          return;
    ...
    test(
      'getFeeRateBps returns a non-negative integer',
      () async {
        if (liveTokenId == null) {
          markTestSkipped('No active market token available');
          return;
    ...
    test(
      'getClobMarketInfo returns CLOB-level market details',
      () async {
        if (liveMarketRef == null) {
          markTestSkipped('No active market available');
          return;
        }
        final info = await client.getClobMarketInfo(liveMarketRef!.conditionId);
        expect(info.conditionId, equals(liveMarketRef!.conditionId));
        expect(info.tokens, isNotEmpty);
        expect(info.tokens.first.tokenId, isNotEmpty);
        expect(info.minimumOrderSize, greaterThanOrEqualTo(0));
        expect(info.minimumTickSize, greaterThan(0));
        expect(info.acceptingOrders, isA<bool>());
        expect(info.negRisk, isA<bool>());
        expect(info.blockaidCheckEnabled, isA<bool>());
        expect(info.raw, contains('t'));
      },
      timeout: const Timeout(Duration(seconds: 10)),
    );
  });
  ...
  group('Price history', () {
    test(
      'getPricesHistory returns a list of price points',
      () async {
        if (liveTokenId == null) {
          markTestSkipped('No active market token available');
          return;
  ...
  group('Error handling', () {
    test(
      'throws PolymarketApiException for invalid condition ID',
      () async {
        expect(
    ...
    test('requiresWallet error is thrown when wallet not set', () {
      expect(() => client.createApiKey(), throwsA(isA<StateError>()));
    });
  ...
  group('Rewards', () {
    ...
    test(
      'getRewardPercentages does not throw a Dart error',
      () async {
        if (!authAvailable) {
          markTestSkipped('No .env credentials');
          return;
        }
        try {
    ...
    test(
      'getCurrentRewards does not throw a Dart error',
      () async {
        if (!authAvailable) {
          markTestSkipped('No .env credentials');
          return;
    ...
    test(
      'getEarningsForDay does not throw a Dart error',
      () async {
        if (!authAvailable) {
          markTestSkipped('No .env credentials');
          return;
    ...
    test(
      'getTotalEarningsForDay does not throw a Dart error',
      () async {
        if (!authAvailable) {
          markTestSkipped('No .env credentials');
          return;
